...
class CustomPromptAttack(Attack):
    """Custom attack class for user-provided prompts."""
    
    def __init__(self, user_prompt, target_model):
        super(CustomPromptAttack, self).__init__(None, target_model)
        
        # Create a single prompt from user input
        self.prompts = [self.create_prompt(user_prompt)]
    
    def create_prompt(self, user_prompt, max_new_tokens=100):
        """Create a prompt from user input."""
        
        # Use the conversation template to format the user prompt
        conv_template = self.target_model.conv_template
